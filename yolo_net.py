...
def loss(yolo_output): #size is None x 7 x 7 x 30, P, X, Y, WIDTH, HEIGHT, P, X, Y, WIDTH, HEIGHT, 20C
    ...
    bndbox1 = yolo_output[0, cell_y, cell_x, 1:5] # [x, y, width, height]
    bndbox1_confidence = yolo_output[0, cell_y, cell_x, 0]
    bndbox2 = yolo_output[0, cell_y, cell_x, 6:10]
...
def iou(box1, box2):
    ...
    intersection_area = (x1-x0+1) * (y1-y0+1)
    pred_area = (predxmax - predxmin + 1) * (predymax - predymin + 1)
